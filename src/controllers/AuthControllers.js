...
async function profile(req, res) {
  ...
  try {
    ...
    permission.map((item) => {
      if (
        !permissions.menus.some(
          (menu) => item.menuName.toLowerCase() === menu.name.toLowerCase()
        )
      ) {
        permissions.menus.push({
          name: item.menuName.toLowerCase(),
          menu: item.menuName.toLowerCase(),
          path: `/${item.role.toLowerCase()}/${item.menuName.toLowerCase()}`,
        });
      }
      if ([...permissions.role].includes(item.role) === false) {
        permissions.role = [...permissions.role, item.role.toLowerCase()];
      }

      permissions.access.push({
        accessName: item.accessName.toLowerCase(),
        permission: {
